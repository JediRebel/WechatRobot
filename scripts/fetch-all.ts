  ...
  for (const config of configsToTest) {
    ...
    try {
      if (isHtmlConfig(config)) {
        // 🚨 核心逻辑：根据 ID 分流到不同的 Puppeteer 脚本或通用爬虫
        if (config.id === "rcmp-nb") {
          await testRcmp(baseOpts)
        } else if (config.id === "town-saint-andrews") {
          await testSaintAndrews(baseOpts) // 处理 403 问题的 Saint Andrews
        } else {
...
// ========== Saint Andrews 特殊 (Puppeteer) ==========
async function testSaintAndrews(opts: ScrapeOptions) {
  console.log(
    "🔍 Testing Town of Saint Andrews (custom dynamic scraper for 403 bypass)",
  )
